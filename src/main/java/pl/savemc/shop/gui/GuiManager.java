...
public class GuiManager {
    ...
    private final Map<GuiID, SMCLayout> guiMap = new HashMap<>();
    ...
    public GuiManager() {
        ...
        shopMenu.setItem(13, Material.EMERALD,       "&5Sklep",   Collections.singletonList("Sklep lore"),
                (player) -> player.openInventory(this.getInventory(GuiID.SHOP)));
        shopMenu.setItem(15, Material.BELL,          "&eBank",    Collections.singletonList("Bank lore"));
                //() -> );

        add(GuiID.SHOP_MENU, shopMenu);
    ...
    private void add(GuiID guiID, SMCLayout smcLayout) {
        smcLayout.createGui();
        guiMap.put(guiID, smcLayout);
    }
